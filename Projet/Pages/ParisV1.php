...
$Match = $bdd->prepare("SELECT p.Somme as somme, m.CoteV1 as V1 FROM Matchs m INNER JOIN Paris p ON m.MatchID = p.MatchID where p.ParisID = ?");
$Match->execute([($_SESSION['ParisID'])]);
$MatchNB = $Match->fetchAll();
// ! au cas ou le gain est deja calculé
if($MatchNB[0]['gain'] == 0){
    $somme = $MatchNB[0]['somme'];
    $V1 = $MatchNB[0]['V1'];
//! renseigne l'attribut gain du paris
